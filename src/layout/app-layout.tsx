...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['store-owner', 'store-administrator', 'inventory-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<StoreInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/stores` : null),
    () => getStores({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Stores',
      path: isTenantUser ? `/stores/view/${data?.[0]?.id}` : '/stores',
      entity: 'store',
    ...
    {
      name: 'Figure Inventories',
      path: '/figure-inventories',
