...
import { getDiscussionBoardById, updateDiscussionBoardById } from 'apiSdk/discussion-boards';
...
function DiscussionBoardEditPage() {
  ...
  const handleSubmit = async (values: DiscussionBoardInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDiscussionBoardById(id, values);
      mutate(updated);
      resetForm();
