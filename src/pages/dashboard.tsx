...
import { getPurchaseHistories } from 'apiSdk/purchase-histories';
...
import { getSalesAnalytics } from 'apiSdk/sales-analytics';
import { SalesAnalyticsInterface } from 'interfaces/sales-analytics';

import { DiscussionBoardListPage as ShowCaseList } from 'pages/discussion-boards';

function DashboardPage() {
  const { session } = useSession();

  const purchaseHistoryStat = useSWR<PaginatedInterface<PurchaseHistoryInterface>>(
    () => `/purchase-histories`,
    () => getPurchaseHistories({ limit: 1, offset: 0 }),
  );

  const figureInventoryStat = useSWR<PaginatedInterface<FigureInventoryInterface>>(
    () => `/figure-inventories`,
    () => getFigureInventories({ limit: 1, offset: 0 }),
  );

  const salesAnalyticsStat = useSWR<PaginatedInterface<SalesAnalyticsInterface>>(
    () => `/sales-analytics`,
    () => getSalesAnalytics({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Purchase Histories',
      result: purchaseHistoryStat?.data?.totalCount,
      isLoading: purchaseHistoryStat.isLoading,
    },
    {
      title: 'Figure Inventories',
      result: figureInventoryStat?.data?.totalCount,
      isLoading: figureInventoryStat.isLoading,
    },
    {
      title: 'Sales Analytics',
      result: salesAnalyticsStat?.data?.totalCount,
      isLoading: salesAnalyticsStat.isLoading,
    },
  ];
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Discussion Boards
                </Text>
                <NextLink href={`discussion-boards`} passHref legacyBehavior>
                  <Button
