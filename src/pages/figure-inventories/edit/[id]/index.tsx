...
import { getFigureInventoryById, updateFigureInventoryById } from 'apiSdk/figure-inventories';
...
function FigureInventoryEditPage() {
  ...
  const handleSubmit = async (values: FigureInventoryInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateFigureInventoryById(id, values);
      mutate(updated);
      resetForm();
